(function ($, window) {
    ...
    function initWebUpload(item, options) {
        ...
        var webuploaderoptions = $.extend({
            ...
            // 文件接收服务端。
            //server: '/Home/AddFile',
            server: 'http://192.168.0.121:9090/upload/',
            deleteServer: 'http://192.168.0.121:9090/upload/remove',
        ...
        uploader.on('uploadSuccess', function (file, response) {//上传成功事件
            ...
            if (response.state == "error") {
                ...
                $hiddenInput.append('<input type="text" id="hiddenInput' + $(item)[0].id + file.id + '" class="hiddenInput" value="' + response.data + '" />')
            }
    ...
    //获取附件字符串数组 去掉最后一个逗号
    $.fn.GetFilesValuesStr = function (options) {
        var ele = $(this);
        var filesdata = ele.find(".UploadhiddenInput");
        var str = "";
        filesdata.find(".hiddenInput").each(function () {
